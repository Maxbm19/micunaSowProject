...
form.addEventListener('submit', function(event) {
  ...
  // Separar latitud y longitud del campo 'zona'
  const [latitude, longitude] = zona.split(',').map(coord => coord.trim());
  ...
  const url = `http://localhost:8086/predicions?latitude=${encodeURIComponent(parseFloat(latitude))}&longitude=${encodeURIComponent(parseFloat(longitude))}`;
  ...
  .then(data => {
    ...
    document.getElementById('precipitacion').innerText =data.precipitacion;
    document.getElementById('probabilidad').innerText = data.probabilidad;
